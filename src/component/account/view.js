    ...
    return (
        ...
        >
            <Typography
                ...
                component="div"
            >
                {props.loading ? <><Typography component="div" key={'body1'} variant={'body1'}>
                    <Skeleton />
                </Typography></> : <>
                    Accounts Detail for  {props.bank.name} Bank
                </>}
            </Typography>
            <Typography
                sx={{ flex: '1 1 100%' }}
            ...
            >
                {props.loading ? <><Typography component="div" key={'body1'} variant={'body1'}>
                    ...
                    Sum Credit Amount: {props.bank.creditAmount}
                </>}
            </Typography>
            <Typography
                sx={{ flex: '1 1 100%' }}
            ...
            >
                {props.loading ? <><Typography component="div" key={'body1'} variant={'body1'}>
                    ...
                    Sum Debit Amount: {props.bank.debitAmount}
                </>}
            </Typography>
            {!props.loading && <>
                <Tooltip title="Add">
...
export default function AccountView(props) {
    ...
    const deleteAccount = function (id) {
        fetch(`http://localhost:3000/bank/${id}/${props.bankId}`, {
            method: "DELETE",
        }).then(res => res.json())
            .then(res => {
    ...
    const editAccount = function (bank) {
    ...
    return (
        ...
        <Box sx={{ width: '100%' }}>
            <Paper sx={{ width: '100%', mb: 2 }}>
                <EnhancedTableToolbar loading={props.loading} bank={props.bank} openModalCallback={() => openModal()} numSelected={selected.length} />
                <TableContainer>
                    <Table
                        sx={{ minWidth: 750 }}
                                ...
                                .map((row, index) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <TableCell align="right"><span>{row.tenure}</span></TableCell>
                                            <TableCell align="right"><span>{row.principal} </span></TableCell>
                                            <TableCell align="right"><span>{row.maturityAmount}</span></TableCell>
                                            <TableCell align="right"><span>{row.totalPayment}</span></TableCell>
